...
export function useParseData2() {
  ...
  async function checkSpenderAllowance({
    ...
    try {
      ...
      console.log('commands-1:', commands.toString())
      console.log('inputs-1:', inputs.toString())
      ...
      if (isUnWrapEth) {
        ...
        console.log('isUnWrapEth-swapCommand', swapCommand)

        if (swapCommand) {
          console.log('isUnWrapEth-swapCommand-in---', swapCommand)
          console.log('isUnWrapEth-swapCommand-inputs---', inputs)
          console.log('isUnWrapEth-swapCommand-commands---', commands)
      ...
      const abi = ['function execute(bytes,bytes[],uint256)']
      const abiInterface = new ethers.utils.Interface(abi)
      const deadlines = await getDeadline(1800)
      const datas = abiInterface.encodeFunctionData('execute', [
        commands,
        inputs,
        deadlines,
      ])
      ...
      let copyTx = await signer?.sendTransaction({
        to: UniversalRouter,
        data: datas,
        value: depositWETH,
      })
